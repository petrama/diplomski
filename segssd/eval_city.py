    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/cpu:0'):
            with tf.name_scope(FLAGS.dataset_name + '_data_provider'):
                (image, gbboxes, glabels,name,seg_labels) \
                        = dataset_provider.inputs(FLAGS.dataset_dir, shuffle=False, num_epochs=False,
        ...
        if FLAGS.max_num_batches:
            ...
            num_batches = math.ceil(dataset_provider.SPLITS_TO_SIZES[FLAGS.dataset_split_name] -6/ float(FLAGS.batch_size))
        ...
        if not FLAGS.wait_for_checkpoints:
            ...
            with tf.Session() as ses:
                saver.restore(ses, checkpoint_path)
