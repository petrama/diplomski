...
def ssd_losses(logits, localisations,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        for i in range(len(logits)):
            predictions.append(tf.nn.softmax(tf.reshape(logits[i], [-1, num_classes])))
        ...
        flogits=tf.concat(flogits,axis=0)
        dtype = flogits.dtype
        ...
        with tf.name_scope('cross_entropy_pos'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=flogits,
                                                                  labels=gclasses)
            loss_pos_cross_entropy = tf.div(tf.reduce_sum(loss * fpmask), n_positives, name='value')
            tf.Print(loss_pos_cross_entropy,[loss_pos_cross_entropy],'Pos CE:')

            tf.losses.add_loss(loss_pos_cross_entropy)

        with tf.name_scope('cross_entropy_neg'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=flogits,
                                                                  labels=gclasses)
            loss_neg_cross_entropy = tf.div(tf.reduce_sum(loss * fnmask), n_positives, name='value')
            tf.Print(loss_neg_cross_entropy,[loss_neg_cross_entropy],'Neg ce:')
            tf.losses.add_loss(loss_neg_cross_entropy)
        ...
        with tf.name_scope('localization'):
            ...
            loss_localization= tf.div(tf.reduce_sum(loss * weights), n_positives, name='value')
            tf.losses.add_loss(loss_localization)

    return loss_pos_cross_entropy,loss_neg_cross_entropy,loss_localization
