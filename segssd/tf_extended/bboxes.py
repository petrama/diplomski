...
def bboxes_filter_center_overlap(labels,bboxes, margins=[0., 0., 0., 0.],scope=None,threshold=0.5):
    with tf.name_scope(scope, 'bboxes_filter', [labels, bboxes]):
        ...
        mask_final=tf.logical_and(mask_thresh,mask)
        labels = tf.boolean_mask(labels, mask_final)
        bboxes = tf.boolean_mask(bboxes, mask_final)
...
def bboxes_filter_overlap(labels, bboxes,
    ...
    """
    with tf.name_scope(scope, 'bboxes_filter', [labels, bboxes]):
        scores = bboxes_intersection(bbox_ref,
                                     bboxes)
