...
tf.app.flags.DEFINE_string('config_path', "config/spp.py", """Path to experiment config.""")
...
def train(model, resume_path=None):
    with tf.Graph().as_default():
        ...
        train_data, train_labels, train_names, _,_,_ = reader.inputs(shuffle=True,
                                                                             num_epochs=FLAGS.max_epochs,
                                                                             dataset_partition='train')


        val_data, val_labels, val_names, _,_,_ = reader.inputs(shuffle=False,
                                                                     num_epochs=FLAGS.max_epochs,
                                                                     dataset_partition='val')


        with tf.variable_scope('model'):
            logits, loss, init_op, init_feed = model.build(train_data, train_labels, None)

        with tf.variable_scope('model', reuse=True):
            logits_val, loss_val = model.build(val_data, val_labels, None, is_training=False)
        ...
        saver = tf.train.Saver(max_to_keep=FLAGS.max_epochs,sharded=False)
