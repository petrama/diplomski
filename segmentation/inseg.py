...
def build(inputs, labels, weights,vector_centr,instance_mask, is_training=True):
    ...
    with tf.contrib.framework.arg_scope([layers.convolution2d], stride=1, padding='SAME',
        ...
        with tf.variable_scope('centers'):
            xss = layers.convolution2d(net, 512, kernel_size=3, scope='conv6_1')
            xss = layers.convolution2d(xss, 512, kernel_size=1, scope='conv6_2' )
    ...
    if is_training:
        ...
        return logits,xss,loss,ce_loss,regr_loss, init_op, init_feed

    return logits,xss,loss,ce_loss,regr_loss
