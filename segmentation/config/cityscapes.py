...
SAVE_DIR=os.path.join('/home/pmarce/results/cityscapes/',train_helper.get_time_string())

flags.DEFINE_string('train_dir',SAVE_DIR,'new folder to save outputs')


flags.DEFINE_string('model_path', 'inseg.py',
                    'path to model definition')


tf.app.flags.DEFINE_string(
    'checkpoint_exclude_scopes', 'model/centers',
    'Comma-separated list of scopes of variables to exclude when restoring '
    'from a checkpoint.')

# used in prepare_tfrecords
flags.DEFINE_string('dataset_dir', '/home/pmarce/datasets/cityscapes/tfrecords',
                    'Directory in which tfrecord files will be saved,contains subdirs train and val')
...
flags.DEFINE_string('vgg_init_dir','/home/pmarce/vgg16/','folder with vgg weights')
...
tf.app.flags.DEFINE_integer('max_epochs', 50, 'Number of batches to run.')

tf.app.flags.DEFINE_string('optimizer','Adam','')

tf.app.flags.DEFINE_float('weight_decay',1e-3,'l2 weight decay for regularization')

tf.app.flags.DEFINE_string('resume_path','/home/pmarce/results/cityscapes/27_6_15-19-55/model.ckpt','')
