...
def _get_output_filename(output_dir, name, filename):
    return '%s/%s_%s.tfrecord' % (output_dir, name, filename)
...
def run(dataset_dir, output_dir, name='voc_train', shuffling=False):
    ...
    #petra
    imgset=name.split('_')[2]
    if imgset == 'train':
        imgset='trainval'

    image_set_file=os.path.join(dataset_dir,'ImageSets','Main',imgset+'.txt')
    ...
    while i < len(filenames):
        ...
        tf_filename = _get_output_filename(output_dir, name, filenames[i])
        with tf.python_io.TFRecordWriter(tf_filename) as tfrecord_writer:
            ...
            while i < len(filenames) and j < SAMPLES_PER_FILES:
                ...
                img_name = filenames[i]#filename[:-4]
